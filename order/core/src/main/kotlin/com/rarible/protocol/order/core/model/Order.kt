    ...
    companion object {
        ...
        fun hashKey(
            ...
            is OrderRaribleV2DataV2, is OrderRaribleV2DataV3 -> {
                ...
                val makeAssetTypeHash = AssetType.hash(makeAssetType)
                val makeAssetTypeHashtoBytes = makeAssetTypeHash.bytes()
                val takeAssetTypeHash = AssetType.hash(takeAssetType)
                val takeAssetTypeHashtoBytes = takeAssetTypeHash.bytes()
                val dataToEthereum = data.toEthereum()
                val dataToEthereumtoBytes = dataToEthereum.bytes()
                val tuples5 = Tuple5(
                    maker,
                    makeAssetTypeHashtoBytes,
                    takeAssetTypeHashtoBytes,
                    salt,
                    dataToEthereumtoBytes
                )
                val encode = Tuples.orderKeyHashTypeDataV2().encode(
                    tuples5
                )

                val result = keccak256(
                    encode
                )
                ...
                val encode = Tuples.orderKeyHashTypeDataV1().encode(
                    tuples
                )
